...
| Frontend Tests on API Change | Runs on pull requests to `main` when there are changes under `services/api/**`, or when root config files that affect the workspace are updated. | Executes the frontend Jest suite to confirm the FE still passes after API-side changes. |
|  |  |  |
|  |  |  |
